...
\begin{proof}
(a)-(e) were just shown.
The equivalence of (a),(f),(h) was shown by Sakai.
The equivalence of (f) and (g) is given in 4.37 of [Comb. of Open Covers in Gen Prog Top III].
\end{proof}
...
%\end{theorem}
%\begin{proof}
%Essentially Theorem 1 of
%[https://doi.org/10.1016/0016-660X(78)90032-6].
%\end{proof}
%
%\begin{lemma}
%If \(\plII\notmarkwin\gruClusGame{X}{x}\),
%then \(X\) is countably tight at \(x\).
%\end{lemma}
%\begin{proof}
%Let \(A\in\Omega_{X,x}\), and assume
%\(\plII\notmarkwin\gruClusGame{X}{x}\).
%Let \(\sigma\) be a Markov strategy
%for \(\plII\) that chooses exclusively from \(A\).
%Since \(\sigma\) is not winning,
%there exist open sets \(U_n\in T_{X,x}\)
%such that \(\{\sigma(\<U_n\>,n):n<\omega\}\in\Omega_{X,x}\).
%This shows that \(X\) is countably tight at \(x\).
%\end{proof}
%
%\begin{theorem}
%TODO
...
%\begin{proof}
%
%So assume (a). If \(X\) is not countably tight at \(x\),
%then we already have (b) by the lemma. So assume \(X\)
%is countably tight at \(x\).
%
%Let \(\tau\) be a winning strategy for \(\plII\).
%Let \(f(\emptyset)=\emptyset\).
%Suppose \(f(s)\in T_{X,x}^{|s|}\) has been defined
%for \(s\in\omega^{<\omega}\). Then
%the set \(\{\tau(s\concat\<U\>):U\in T_{X,x}\}\in\Omega_{X,x}\),
%so define \(f(s\concat\<n\>)\in T_{X,x}^{|s|+1}\) for \(n<\omega\)
%such that \(\{\tau(f(s\concat\<n\>)):n<\omega\}\in\Omega_{X,x}\).
%
%Let \(b:\omega\to\omega^\omega\), and
%let \(\sigma(U,m)\in\{\tau(f(b(m)\concat\<n\>:n<\omega\}\cap U\).
%Then for any attack \(a\in T_{X,x}^\omega\),
%
